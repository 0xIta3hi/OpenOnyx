        ...
        for (const result of results) {
          ...
          lines.forEach((line, lineIndex) => {
            ...
            while ((match = namePattern.exec(line)) !== null) {
              ...
              if (afterMatch.includes(']]') && !afterMatch.includes('[[')) continue;
              ...
              foundMentions.push({
                ...
                matchStart: match.index - contextStart + (contextStart > 0 ? 3 : 0),
                matchEnd: match.index - contextStart + match[0].length + (contextStart > 0 ? 3 : 0),
              });
            }
          });
        }
              ...
              {Array.from(groupedMentions.entries()).map(([path, fileMentions]) => (
                <div key={path} className="unlinked-file">
                  <div className="unlinked-file-header">
                    ...
                    <span className="mention-count">{fileMentions.length}</span>
                  </div>
                  {fileMentions.map((mention, i) => (
                    ...
                    >
                      <span className="mention-context">
                        ...
                        <mark>{mention.context.substring(mention.matchStart, mention.matchEnd)}</mark>
