...
export function SearchModal({ onClose, onSelect, recentFiles = [], starredNotes = [], fileTree = [] }: SearchModalProps) {
  ...
  const displayItems = useMemo(() => {
    if (mode === 'search') {
      return results.map(r => ({ name: getNoteName(r.name), path: r.path, match: r.matches[0]?.value }));
    } else {
      ...
      if (query.trim()) {
        ...
        const items: { name: string; path: string; isStarred?: boolean; isRecent?: boolean }[] = [];
      ...
      <div className="search-modal" onClick={e => e.stopPropagation()}>
        <div className="search-mode-tabs">
          <button 
            ...
            onClick={() => { setMode('switcher'); setSelectedIndex(0); }}
          >
        ...
        <div className="search-input-wrapper">
          ...
          <input
            ...
            type="text"
            placeholder={mode === 'search' ? 'Search note contents...' : 'Quick switch to note...'}
            value={query}
          ...
          {displayItems.length > 0 ? (
            displayItems.map((item: any, index) => (
              ...
              >
                <span className="result-icon">
                  {item.isStarred ? <Star size={14} fill="var(--accent-warning)" stroke="var(--accent-warning)" /> : 
                   item.isRecent ? <Clock size={14} /> : <FileText size={14} />}
                </span>
            ...
            <div className="empty-state" style={{ padding: '2rem' }}>
              <div className="empty-text">
                {mode === 'search' ? 'Start typing to search contents...' : 
                 recentFiles.length === 0 && starredNotes.length === 0 ? 'Start typing to find notes...' :
