...
export function TitleBar({ theme, onCommandPalette, commands = [] }: TitleBarProps) {
  const api = getAPI();
  const isMac = navigator.platform.includes('Mac');
  const [activeMenu, setActiveMenu] = useState<string | null>(null);
  const menuRef = useRef<HTMLDivElement>(null);

  const categories = Array.from(new Set(commands.map(cmd => cmd.category || 'Other')));
  ...
  return (
    <div className="titlebar">
      <div className="titlebar-left" style={{ display: 'flex', alignItems: 'center' }}>
        <div className="titlebar-title" style={{ marginRight: '16px' }}>
          OpenObsidian
        </div>
        
        {commands.length > 0 && (
          <div className="titlebar-menu" ref={menuRef} style={{ display: 'flex', WebkitAppRegion: 'no-drag' } as any}>
            {categories.map(category => (
              <div key={category} style={{ position: 'relative' }}>
                <button 
                  ...
                  style={{
                    background: activeMenu === category ? 'var(--bg-active)' : 'transparent',
                    border: 'none',
                ...
                {activeMenu === category && (
                  <div className="menu-dropdown" style={{
                    ...
                    {commands.filter(cmd => (cmd.category || 'Other') === category).map(cmd => (
                      <button
                        key={cmd.id}
                        className="menu-item"
                        onClick={() => {
